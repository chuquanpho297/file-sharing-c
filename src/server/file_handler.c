...
void handle_create_file(client_t* client, const char* buffer) {
    ...
    char* parent_id = get_root_folder_id(client->username);
    char* folder_name = NULL;
    
    while (token != NULL) {
        folder_name = token;
        parent_id = get_folder_id(folder_name, client->username, parent_id);
        if(parent_id == NULL){
    ...
    if (create_file(file_name, file_size, parent_id, client->username)) {
...
void handle_copy_file(client_t* client, const char* buffer) {
    ...
    while (token != NULL) {
        ...
        if (token == NULL) {
            ...
            parent_id = get_folder_id(token, client->username, parent_id);
            if(parent_id == NULL){
                send_response(client->socket, 404, "Folder not found");
                json_object_put(parsed_json);
                return;
    ...
    char* to_folder_id = get_root_folder_id(client->username);
    while (token != NULL) {
        to_folder_id = get_folder_id(token, client->username, to_folder_id);
...
void handle_download_file(client_t* client, const char* buffer) {
    ...
    char* token = strtok((char*)file_path, "/");
    char* parent_id = get_root_folder_id(file_owner);
    char* file_id = NULL;
    char* file_name = NULL;
    ...
    while (token != NULL) {
        ...
        if (token == NULL) {
            ...
            if(get_file_access(file_id) == 'download'){
            ...
            parent_id = get_folder_id(token, file_owner, parent_id);
            if(parent_id == NULL){
                send_response(client->socket, 404, "Folder not found");
                json_object_put(parsed_json);
                return;
    ...
    //     struct json_object *response = json_object_new_object();
    //     json_object_object_add(response, "responseCode", json_object_new_int(200));
    //     json_object_object_add(response, "message", json_object_new_string("File downloaded successfully"));
